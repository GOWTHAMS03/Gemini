...
const navigationGroups: NavGroup[] = [
  {
    ...
    icon: PieChart,
    items: [
      { name: 'Overview', to: '/', icon: PieChart },
    ],
  },
  {
    ...
    icon: Users,
    items: [
      { name: 'Employee Onboarding', to: '/employees', icon: Users },
      { name: 'Customer Onboarding', to: '/shops', icon: Store },
    ],
  },
  {
    ...
    icon: Package,
    items: [
      { name: 'Category', to: '/categories', icon: Tag },
      { name: 'Product Catalog', to: '/products', icon: Package },
      { name: 'Production', to: '/production', icon: Factory, badge: 'BETA' },
      { name: 'Recipes / BOM', to: '/recipes', icon: UtensilsCrossed, badge: 'BETA' },
      { name: 'Raw Materials Inventory', to: '/raw-materials', icon: Boxes, badge: 'BETA' },
    ],
  },
  {
    ...
    items: [
      ...
      { name: 'Delivery Route', to: '/routes', icon: MapPin },
      { name: 'Trip Dispatch', to: '/trip-dispatch', icon: Navigation },
      { name: 'Weekly Trip Planning', to: '/weekly-trip-planning', icon: CalendarDays },
      { name: 'Shop Visit', to: '/shop-visits', icon: Store },
    ],
  },
  {
    label: 'Sales & Finance',
    icon: DollarSign,
    items: [
      { name: 'Sales & Invoices', to: '/invoices', icon: FileText },
      { name: 'Returns & Replacement', to: '/returns', icon: Sparkles },
      { name: 'Finance Dashboard', to: '/finance', icon: DollarSign },
      ...
      { name: 'Financial Reports', to: '/financial-reports', icon: FileSpreadsheet },
              ...
              if (group.items.length === 1) {
                ...
                return (
                  <NavLink
                    ...
                    to={item.to}
                    onClick={handleNavClick}
                    className={`flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-semibold transition cursor-pointer ${
                      isActive
                ...
                <div key={group.label} className="space-y-0.5">
                  ...
                  {!collapsed ? (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-xl text-[11px] font-extrabold uppercase tracking-wider transition cursor-pointer ${
                        hasActiveChild
                      ...
                      {group.items.map(item => {
                        ...
                        return (
                          <NavLink
                            ...
                            to={item.to}
                            onClick={handleNavClick}
                            className={`flex items-center gap-3 px-3 py-1.5 rounded-xl text-xs font-semibold transition cursor-pointer ${
                              isActive
                                ? 'bg-indigo-600 text-white shadow-sm font-bold'
                                : 'text-[#1C1C1C] dark:text-slate-300 hover:bg-[#F4F5F7] dark:hover:bg-slate-800/80'
                            } ${collapsed ? 'justify-center px-0' : ''}`}
                          >
                            ...
                            {!collapsed && (
                              <div className="flex items-center justify-between flex-1 min-w-0">
                                <span className="truncate">{item.name}</span>
                                {item.badge && (
                                  <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded-md tracking-wider ${
                                    isActive
