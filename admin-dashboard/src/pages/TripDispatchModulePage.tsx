...
import api, { routeApi, salesDeliveryApi, tripApi, ApiRouteShop } from '../services/apiService';
...
export const TripDispatchModulePage: React.FC = () => {
  ...
  // Sales Person Trip Lifecycle: Start & End Trip Verification State
  ...
  const handleStartTripConfirmed = async () => {
    ...
    try {
      ...
      showToast(`🚀 Trip #${tripToStart.tripNumber} started successfully! Sales route is now Active.`);
      setTripToStart(null);
      setStartVerificationChecked(false);
      fetchActiveTrips();
      if (inspectingTrip?.id === tripToStart.id) {
        setInspectingTrip(prev => prev ? { ...prev, status: 'IN_PROGRESS' } : null);
      }
    } catch (err: any) {
      console.error('Failed to start trip:', err);
      showToast(err.response?.data?.message || 'Failed to start trip');
    } finally {
  ...
  const handleEndTripConfirmed = async () => {
    ...
    try {
      ...
      showToast(`🏁 Trip #${tripToEnd.tripNumber} completed! Inventory and collections reconciled.`);
      setTripToEnd(null);
      setEndVerificationChecked(false);
      fetchActiveTrips();
      if (inspectingTrip?.id === tripToEnd.id) {
        setInspectingTrip(prev => prev ? { ...prev, status: 'COMPLETED' } : null);
      }
    } catch (err: any) {
      console.error('Failed to end trip:', err);
      showToast(err.response?.data?.message || 'Failed to complete trip');
    } finally {
            ...
            filteredTrips.map((trip) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Card Action Buttons (Start Trip / End Trip with Verification) */}
                  <div className="pt-3 border-t border-[#F0F2F5] dark:border-slate-700 flex items-center gap-1.5 flex-wrap">
                    {trip.status !== 'IN_PROGRESS' && trip.status !== 'COMPLETED' && (
                      <button
                        ...
                        className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer shadow-xs active:scale-95"
                        title="Verify and Start Route Trip"
                      >
                    ...
                    {trip.status === 'IN_PROGRESS' && (
                      <button
                        ...
                        className="flex-1 py-2 bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer shadow-xs active:scale-95"
                        title="Verify and End Route Trip"
                      >
                    ...
                    {trip.status === 'COMPLETED' && (
                      <span className="flex-1 py-1.5 text-center text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 rounded-xl border border-emerald-200 dark:border-emerald-800 flex items-center justify-center gap-1">
                        <Check className="w-3.5 h-3.5" /> Trip Finished
                      </span>
                    )}
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-[#1C1C1C] dark:bg-slate-700 hover:bg-black text-white font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer shadow-xs active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="p-2 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 text-rose-600 dark:text-rose-400 font-bold rounded-xl text-xs transition cursor-pointer border border-rose-200 dark:border-rose-800 shrink-0"
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          {trip.status !== 'IN_PROGRESS' && trip.status !== 'COMPLETED' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer shadow-2xs"
                              title="Start Trip"
                            >
                          ...
                          {trip.status === 'IN_PROGRESS' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer shadow-2xs"
                              title="End Trip"
                            >
                          ...
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-[#1C1C1C] dark:bg-slate-700 text-white rounded-xl text-xs font-bold cursor-pointer"
                          >
                            Inspect
                          </button>
                          <button
                ...
                <>
                  ...
                  <div className="p-5 sm:p-6 bg-white dark:bg-slate-800 border-b border-[#F0F2F5] dark:border-slate-700 flex flex-col sm:flex-row justify-between sm:items-center gap-4 shrink-0">
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-center">
                      {inspectingTrip.status !== 'IN_PROGRESS' && inspectingTrip.status !== 'COMPLETED' && (
                        <button
                          type="button"
                          onClick={() => {
                            setTripToStart(inspectingTrip);
                            setStartVerificationChecked(false);
                          }}
                          className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-xl text-xs flex items-center gap-1.5 shadow-xs cursor-pointer active:scale-95"
                        >
                          <Play className="w-3.5 h-3.5 fill-current" /> Start Trip Now
                        </button>
                      )}

                      {inspectingTrip.status === 'IN_PROGRESS' && (
                        <button
                          type="button"
                          onClick={() => {
                            setTripToEnd(inspectingTrip);
                            setEndVerificationChecked(false);
                          }}
                          className="px-3.5 py-2 bg-purple-600 hover:bg-purple-700 text-white font-extrabold rounded-xl text-xs flex items-center gap-1.5 shadow-xs cursor-pointer active:scale-95"
                        >
                          <CheckCircle2 className="w-3.5 h-3.5" /> End & Reconcile Trip
                        </button>
                      )}
      ...
      {tripToStart && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[999999] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-700 space-y-4 animate-in fade-in zoom-in duration-200">
            ...
            {/* Trip Pre-Flight Verification Dossier */}
            <div className="p-4 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2.5 text-xs">
              ...
              <div className="flex justify-between items-center">
                ...
                <strong className="text-indigo-600 font-bold">{tripToStart.salesPersonName || 'Assigned Executive'}</strong>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Planned Customer Outlets:</span>
                <span className="text-blue-600 font-black">{tripToStart.shopVisits?.length || tripToStart.totalShops || 0} Stops</span>
              </div>
            </div>
      ...
      {tripToEnd && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[999999] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-700 space-y-4 animate-in fade-in zoom-in duration-200">
            ...
            {/* Trip EOD Reconciliation Dossier */}
            {(() => {
              const stats = getTripVisitStats(tripToEnd);
              return (
                <div className="p-4 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3 text-xs">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-2.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                      <span className="text-[10px] text-slate-400 font-bold uppercase block">Visits Completed</span>
                      <strong className="text-emerald-600 text-sm font-mono">{stats.completedCount} / {stats.totalCount} ({stats.progressPct}%)</strong>
                    </div>
                    <div className="p-2.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                      <span className="text-[10px] text-slate-400 font-bold uppercase block">Products Delivered</span>
                      <strong className="text-purple-600 text-sm font-mono">{stats.totalSoldUnits} Pkts</strong>
                    </div>
                    <div className="p-2.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                      <span className="text-[10px] text-slate-400 font-bold uppercase block">Total Billed</span>
                      <strong className="text-blue-600 text-sm font-mono">₹{stats.totalBilled.toLocaleString()}</strong>
                    </div>
                    <div className="p-2.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                      <span className="text-[10px] text-slate-400 font-bold uppercase block">Collected Cash / UPI</span>
                      <strong className="text-emerald-600 text-sm font-mono">₹{stats.totalCollected.toLocaleString()}</strong>
                    </div>
                  </div>

                  <div className="flex justify-between items-center text-[11px] pt-1 font-medium text-slate-600 dark:text-slate-300">
                    <span>Remaining Truck Inventory:</span>
                    <strong className="font-mono text-amber-600">{stats.remainingUnits} Pkts returning to Warehouse</strong>
                  </div>
                </div>
              );
            })()}
            ...
            <label className="flex items-start gap-2.5 p-3 bg-purple-50 dark:bg-purple-950/30 rounded-2xl border border-purple-200 dark:border-purple-800 cursor-pointer">
              ...
              <span className="text-xs text-purple-900 dark:text-purple-200 leading-snug">
                I verify that all shop invoices, collected cash/UPI, and remaining truck bread returns have been reconciled for this trip.
              </span>
            </label>
